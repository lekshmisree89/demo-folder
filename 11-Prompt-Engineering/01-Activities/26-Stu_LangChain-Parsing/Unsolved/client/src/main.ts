...
const createCard = (data: any) => {
  ...
  const jsonString = JSON.stringify(data.response, null, 2);
  const jsonString2 = JSON.stringify(data.formattedResponse, null, 2);
  cardBodyEl.innerHTML = `<p><b>Prompt</b>:</p><p>${data.prompt}</p><p><b>Response</b>:</p><p><pre><code>${jsonString}</code></pre></p><p><b>Formatted Response</b>:</p><p><pre><code>${jsonString2}</code></pre></p>`;
