...
> {
  ...
  declare isDonor: boolean;
  // ! By using the ForeignKey type, we can specify that the driverId field is a foreign key that references the id field of the Driver model. This also means that we do not need to initialize the driverId field in the LicenseFactory.
  declare driverId: ForeignKey<Driver['id']>;//
  // It's done when the `License` model includes a foreign key to the `Driver` model.
}
