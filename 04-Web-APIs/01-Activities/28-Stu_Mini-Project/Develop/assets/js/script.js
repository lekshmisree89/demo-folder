...
//that are added to the mood board and their positions
//DATA
...
function updateLocalStorage() {
  ...
  localStorage.setItem('moodBoard', JSON.stringify(tempStorageObject));
}


// ? Function to load from local storage. This function will 
//be called on page load.
function loadFromLocalStorage() {

  const storedData = JSON.parse(localStorage.getItem('moodBoard'))
  // TODO: Load and parse the data from local storage 
  //and paint the images and text on the mood board
  ...
  if (storedData) {
    ...
    for (let i = 0; i < tempStorageObject.images.length; i++) {
      const image = tempStorageObject.images[i];
    ...
    for (let i = 0; i < tempStorageObject.text.length; i++) {
      const text = tempStorageObject.text[i];
...
//  ? We create an event listener for the image URL input field. 
//This will create an image element and attach it t
//o the mood board with the URL provided by the user.
 // TODO: Create an image element, 
    //add a class of draggable, //
    //set the src attribute to the image URL provided by the user, 
...
addImageBtn.addEventListener('click', function () {
  ...
  if (imageUrl) {
    ...
    //TODO: Set the `currentElement` to the image element you creat
    currentElement = img;
...
addTextBtn.addEventListener('click', function () {
  ...
  // TODO: Attach the mouse move event listener to the document 
  //and the click listener to the mood board div so that the element 
  //can be dragged anywhere on the screen, but dropped only on the mood board div.
...
function placeElementClickHandler(event) {
  ...
  if (currentElement) {
    ...
    // ? Set the position of the element based on the 
    //calculated position above.
